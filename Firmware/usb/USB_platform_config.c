...
{
  uint32_t b_size = Send_length;
  uint32_t b_index = 0;
  uint32_t actual_input_size;  
  
  while (b_index < Send_length)
  {
    if (b_size < (VIRTUAL_COM_PORT_DATA_SIZE)-1)
    {
      actual_input_size = b_size;
    }
    else
    {
      actual_input_size = (VIRTUAL_COM_PORT_DATA_SIZE-1);       
    }
    ...
    UserToPMABufferCopy((unsigned char*)(ptrBuffer+b_index), ENDP1_TXADDR,actual_input_size);
